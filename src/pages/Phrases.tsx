...
export default function Phrases({ entries, progress, onProgressUpdate, onGoToPractice, initialStatus }: PhrasesProps) {
  ...
  const phrases = useMemo(() => entries.filter(e => e.type === 'phrase'), [entries]);

  const categoryOptions = useMemo(
    () => Array.from(new Set(phrases.map(e => e.category).filter(Boolean))),
    [phrases]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [phrases, search, category, status, difficulty, sort, progress]);
      ...
      <div className="flex-1 min-w-0 space-y-4">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-sm text-[var(--color-text-secondary)]">
              Освоено: <span className="font-bold text-[var(--color-success)]">{masteredCount}</span> / {phrases.length}
            </div>
          </div>
        </div>

        <ProgressBar value={masteredCount} max={phrases.length} showPercent color="var(--color-success)" />
        ...
        <div className="flex gap-2 flex-wrap">
            <button
              ...
              onClick={() => {
                const notMastered = phrases.filter(e => {
                  const p = progress[e.id];
        ...
        <div className="space-y-3">
          {filtered.map(entry => {
            ...
            const isOfficial = entry.category === 'official' || entry.category === 'office';
            ...
            return (
              <div key={entry.id} className={`card card-hover transition-all duration-300 ${fbClass} ${fb ? 'scale-[1.02]' : ''} ${isOfficial ? 'border-l-4 border-l-[var(--color-primary)]' : ''}`}>
                ...
                {(entry.example_kz || entry.example_ru) && (
                  <div className="mt-2 p-3 rounded-lg bg-[var(--color-bg)] text-sm border border-[var(--color-border)]">
